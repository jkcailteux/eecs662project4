...
(define parse-cfwaes
  (lambda (expr)
    (cond ((symbol? expr) (id expr))
          ...
          ((list? expr)
           (case (car expr)
             ((-) (cond ((fun? (parse-cfwaes (cadr expr))) 
                         error  'parse-cfwaes "arithmetic operation cannot be performed on a function")
                        ((fun? (parse-cfwaes (caddr expr)))
                         error  'parse-cfwaes "arithmetic operation cannot be performed on a function")
                        (else (sub (parse-cfwaes (cadr expr)) (parse-cfwaes (caddr expr))))))
             ((+) (cond ((fun? (parse-cfwaes (cadr expr))) 
                         error  'parse-cfwaes "arithmetic operation cannot be performed on a function")
                        ((fun? (parse-cfwaes (caddr expr)))
                         error  'parse-cfwaes "arithmetic operation cannot be performed on a function")
                        (else (add (parse-cfwaes (cadr expr)) (parse-cfwaes (caddr expr))))))
             ((*) (cond ((fun? (parse-cfwaes (cadr expr))) 
                         error  'parse-cfwaes "arithmetic operation cannot be performed on a function")
                        ((fun? (parse-cfwaes (caddr expr)))
                         error  'parse-cfwaes "arithmetic operation cannot be performed on a function")
                        (else (mul (parse-cfwaes (cadr expr)) (parse-cfwaes (caddr expr))))))
             ((/) (cond ((fun? (parse-cfwaes (cadr expr))) 
                         error  'parse-cfwaes "arithmetic operation cannot be performed on a function")
                        ((fun? (parse-cfwaes (caddr expr)))
                         error  'parse-cfwaes "arithmetic operation cannot be performed on a function")
                        (else (div (parse-cfwaes (cadr expr)) (parse-cfwaes (caddr expr))))))((if0) (if0 (parse-cfwaes (cadr expr)) (parse-cfwaes(caddr expr))
                                                                                                         (parse-cfwaes (cadddr expr))))
             ((fun) (if (number? (cadr expr))
                        (error  'parse-cfwaes "cannot call a number like a function")
                        (fun (cadr expr) 
